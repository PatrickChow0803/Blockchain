...
def mine():
    ...
    # Pulls the data out of the POST
    data = request.get_json()

    if "id" not in data or "proof" not in data:
        return {"message": "required field missing"}, 400

    valid = blockchain.valid_proof(json.dumps(prev, sort_keys=True), data["proof"])
    if "id" in blockchain.last_block:
        valid = False
    if valid:
        blockchain.last_block["id"] = data["id"]
        blockchain.new_block(data["proof"], blockchain.hash(prev))
